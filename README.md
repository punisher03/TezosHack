...
* We developed a the escrow smart contact but we ran into problems when trying to invoke it, So you can test the smart contact by going to `smartpy.io` and pasting our code which is present in the root folder
* Our product is not yet complete You can try out the transaction feature by logging in as a buyer and clicking on a product to buy or to simply testing tthe transaction feature you can go to `http://localhost:3000/transaction` in  your browser after starting the react app.
